...
def generaterReport(project):
    global html
    data = ""
    with open(f"{资产收集}\\Web-SurvivalScan\\.data\\report.json",encoding="utf-8",mode="r+") as file:
        data = file.read()
        
    html = html.replace("{{}}",data)
    with open(f"{资产收集}\\{project}\\存活报告.html",encoding="utf-8",mode="+w") as file:
        file.write(html)
        cprint(f"[+][+][+] 已经将存活目标生成至HTML，导出至 report.html","red")

if __name__ == '__main__': 
    project= args[1]
    generaterReport(project)
